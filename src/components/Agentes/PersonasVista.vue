...
async function grabaRegistro() {
 ...
 /* se elimina la restricción de no grabar cbu sin cuenta
 if (CBU > 0) {
    if (CUENTA.length == 0) {
  ...
  <v-container>
    <v-card>
      <v-form ref="form" v-model="formOK">
        <v-card-title>Detalle de la Persona</v-card-title>
        <v-card-subtitle>
          {{ registroActual.ID == 0 ? 'Agregar ' : 'Datos de la persona' }}
        </v-card-subtitle>
        <v-alert
            ...
            <v-row>
              ...
              <v-col cols="2">
                <v-text-field
                  ...
                  label="Número"
                  lazy-validation
                  :rules="[...rules.number, (val) => rules.longitudMaxONull(val, 10)]"
                ></v-text-field>
              </v-col>
              <v-col cols="2">
                <v-text-field
